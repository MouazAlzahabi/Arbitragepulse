...
pub fn apply_log_to_cache(
    ...
    } else if topic0 == Some(IPancakeV3Pool::Swap::SIGNATURE_HASH) {
        if let Ok(decoded) = IPancakeV3Pool::Swap::decode_log(log.as_ref()) {
            cache.update_v3_state(pool, U256::from(decoded.sqrtPriceX96), decoded.liquidity.into());
            return true;
        }
    }
...
impl Listener {
    ...
    pub async fn subscribe<P: Provider + Clone + 'static>(
        ...
        let event_sigs = vec![
            ...
            IPancakeV3Pool::Swap::SIGNATURE_HASH,  // PancakeSwap V3: Swap(9 fields, with protocol fees)
        ];
                ...
                match provider.get_logs(&filter).await {
                    Ok(logs) => {
                        for log in logs {
                            ...
                            if topic0 == Some(Sync::SIGNATURE_HASH) {
                            ...
                            } else if topic0 == Some(Swap::SIGNATURE_HASH) || topic0 == Some(IPancakeV3Pool::Swap::SIGNATURE_HASH) {
                                ...
                                let v3_decoded = if topic0 == Some(Swap::SIGNATURE_HASH) {
                                    ...
                                    IPancakeV3Pool::Swap::decode_log(log.as_ref())
                                        .ok()
                                        .map(|d| (U256::from(d.sqrtPriceX96), d.liquidity.into()))
                                };
