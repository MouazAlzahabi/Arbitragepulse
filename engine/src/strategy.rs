            ...
            if let Some(amount_back) = amount_back_opt.filter(|&b| !b.is_zero()) {
                ...
                if amount_in_f64 > 0.0 {
                    ...
                    if spread > P15_GATE_SPREAD && matches!(task.router_a_type, RouterType::V3) {
                        // Skip heavily tick-capped pools (< 20% of trade size). These show
                        // phantom spreads at tiny scale that vanish at full trade size, so
                        // QuoterV2 would just confirm "no opp" every single scan. The other
                        // fee tiers for the same router are already detected independently
                        // via their own Phase 1 tasks — no expansion needed.
    ...
    pub async fn detect_triangular<P: Provider + Clone>(
        ...
        // re-chain B→C and C→A locally to find real profit at the intended capital.
        {
            ...
            for p3e in &p3_entries {
                ...
                // Skip uncapped legs (QuoterV2 not needed) and heavily-capped legs
                // (< 20% capacity — phantom spread at tiny scale, QuoterV2 confirms
